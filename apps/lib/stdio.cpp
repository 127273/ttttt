...
FILE* fopen(char* filename, const char* mode) {
  ...
  uint64_t result = 1;
  if (!strcmp(mode, "r")) {
    syscall(SYSCall_file_open, filename, FILE_MODE_READ, file.scratch_sector, &file.file_info, &result);
    if (result) {
      DEBUG("stdio.c: error opening file %s\n", filename);
      return NULL;
    }
  } else if (!strcmp(mode, "w")){
    syscall(SYSCall_file_open, filename, FILE_MODE_WRITE, file.scratch_sector, &file.file_info, &result);
    if (result) {
      DEBUG("stdio.c: error opening file %s\n", filename);
      return NULL;
    }
