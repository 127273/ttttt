...
kv_file_t* create_empty_kv() {
	kv_file_t* kv = (kv_file_t*)malloc(sizeof(kv_file_t));
	kv->entry_count = 0;
	kv->keys = (char**)malloc(sizeof(char*) * MAX_PAIRS);
	kv->values = (char**)malloc(sizeof(char*) * MAX_PAIRS);
	return kv;
}
...
void open_kv_file(char* filename, kv_file_t* kv_file_out) {
	...
	char** keys_holder = (char**)malloc(sizeof(char*) * MAX_PAIRS);
	char** values_holder = (char**)malloc(sizeof(char*) * MAX_PAIRS);
	char** keys = keys_holder;
	char** values = values_holder;
	char* key_holder = (char*)malloc(sizeof(char) * 64);
	char* value_holder = (char*)malloc(sizeof(char) * 64);
	char* key = key_holder;
...
void create_kv_file(char* filename, kv_file_t* kv_file_data) {
	...
	uint8_t *buffer = (uint8_t*)malloc(sizeof(char) * size + 1);
	uint8_t *iter = buffer;
	for (i = 0; i < kv_file_data->entry_count; i++) {
		char next;
