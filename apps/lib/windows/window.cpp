...
context_t *window_context_create(int width, int height, int bpp) {
	...
	context->bpp = bpp;
	context->buffer = (uint32_t*)malloc(context_size);
...
window_t *window_create(int x, int y, int width, int height, char* title, int id, uint32_t attributes, uint32_t frame_flags) {
	...
	// Create Ext params
	window_ext_t *ext = (window_ext_t*)calloc(sizeof(window_ext_t), 1);
	ext->context = window_context_create(width, height, 32);
	ext->frame_flags = frame_flags;
	window->ext = ext;
	
	// Create window (systemcall)
	//syscall(SYSCall_windows_create, &window);
	int t = x;
	syscall(SYSCall_windows_create, &t, y, width, height, attributes);
	window->handle = t;
	
	DEBUG("window->handle %d  %x\n", window->handle, &window->handle);
	...
	if (!(frame_flags & WINDOW_FRAME_NONE)) {
		...
		bmp_image_t *bmp_close = (bmp_image_t*)malloc(sizeof(bmp_image_t));
		...
		bmp_image_t *bmp_min = (bmp_image_t*)malloc(sizeof(bmp_image_t));
