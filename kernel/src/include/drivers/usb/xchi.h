...
struct xhci_ep_ctx *xhci_get_ep0_ctx(struct xhci_ctx *ctx, uint32_t ctx_size);
...
void fill_trb_buf(struct xhci_command_trb *cmd, uint32_t field1, uint32_t field2, uint32_t field3, uint32_t field4, int first);
int xhci_send_command(struct xhci_hcd *controller, uint32_t field1, uint32_t field2, uint32_t field3, uint32_t field4, struct xhci_event *ev);
int xhci_enable_slot(struct xhci_hcd *controller);
struct xhci_slot_ctx *xhci_get_slot_ctx(struct xhci_ctx *ctx, uint32_t ctx_size);
int xhci_send_addr_device(struct xhci_hcd *xhcd, uint32_t slot_id, uint64_t dma_in_ctx, int bsr);
int xhci_configure_ep(struct xhci_hcd *xhcd, uint32_t slot_id, uint64_t dma_in_ctx);
int xhci_init_device(struct xhci_hcd *controller, uint32_t port, struct xhci_port_protocol proto);
...
namespace myos
{
    namespace drivers
    {
        class XHCIController:  public UsbBaseClass, public myos::hardware::InterruptHandler, public myos::drivers::Driver
        {
        ...
        XHCIController(uintptr_t xhciBaseAddress,uint8_t interruptLine,  myos::hardware::InterruptManager* manager);
        void HandleInterrupt(isr_ctx_t *regs);
        void Activate();

        bool SendCommand(void* data);
        bool TakeOwnership();
