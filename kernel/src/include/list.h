...
public:
	...
	void add_back(const T& obj) {
		...
		if(cache.get_length() <= 0){
			node = (ListNode<T>*)malloc(sizeof(ListNode<T>));
		} else {
    ...
    void add_back_unlocked(const T& obj) {
		...
		ListNode<T>* node;
		if(cache.get_length() <= 0){
			node = (ListNode<T>*)malloc(sizeof(ListNode<T>));//need changed marked	kmalloc_unlocked(sizeof(ListNode<T>));
		} else {
			node = cache.remove_at(0);
		}
	...
	void allocate(unsigned count){
		...
		while(count--){
			cache.add_back((ListNode<T>*)malloc(sizeof(ListNode<T>)));
		}
