...
void init_mmu_frames(uint64_t memory_size) {
  ...
  DEBUG("MMU[frames]: Frames %i, %i kernel_end: %x heapstart: %x \n", mmu_nframes, INDEX_FROM_BIT(mmu_nframes), &kernel_end, heap_start);
  ...
  //heap_start = TO_VMA_U64(0x200000);
  //heap_end = TO_VMA_U64(0x5000000);
  //heap_end = TO_PHYS_U64(heap_end);
  //DEBUG("heap %x", heap_end);
  // MARK USED MEMORY
  // Dummy way : mark everything from 0x0000 - KERNEL_END (heap end)
  uint64_t kernel_end_p = TO_PHYS_U64(heap_end);//heap_end
  uint64_t num_pages = kernel_end_p / PAGE_SIZE + 1;
  for (uint64_t i=0; i<num_pages; i++) {
    mmu_frame_set(i * PAGE_SIZE);
  }
  DEBUG("MMU[frames]: Used Mem %dMB %x\n", TO_PHYS_U64(heap_end) / 1024 / 1024, heap_end);
}