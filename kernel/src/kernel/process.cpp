...
task_t* create_task_struct(bool create_pde) {
  ...
  new_task->kstack = (uint8_t*)malloc(KERNEL_STACK_SIZE);
  if (create_pde) {
    new_task->pde = (pde_t*)malloc(sizeof(pde_t) * 512);
  }
  return new_task;
}
...
uint64_t load_elf(void* elf_data_buffer, void* process_buffer) {
  Elf64_Ehdr *header = (Elf64_Ehdr*)elf_data_buffer;
...
task_t* create_user_process(void* elf_raw_data) {
  ...
  uint64_t phys_addr;
  uint8_t* task_memory = (uint8_t*)alloc_frame_temp(&phys_addr);
...
static void free_allocated_frames(task_t *task) {
  for (int i=0; i<512; i++) {
    if (task->pde[i].all != 0) {//no change 3
...
void kill_process_no_schedule(task_t* task) {
  ...
  // Close window if any
  if (task->window != NULL) {
    window_close((window_t*)task->window);
  }
...
extern "C" void __switch_to(task_t* next) {
