...
//		volinfo->system[8] = 0; 
	...
	if(volinfo->numsecs - volinfo->dataarea > 0)
		volinfo->numclusters = (volinfo->numsecs - volinfo->dataarea) / volinfo->secperclus;
	else 
	if(volinfo->numsecs - volinfo->dataarea > 0)
		volinfo->numclusters = (volinfo->dataarea - volinfo->numsecs ) / volinfo->secperclus;
...
*/
uint32_t DFS_ReadFile(PFILEINFO fileinfo, uint8_t *scratch, uint8_t *buffer, uint32_t *successcount, size_t len)
{
