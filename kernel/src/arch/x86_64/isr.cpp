...
void InterruptManager::handleInterrupt(isr_ctx_t *regs)
{
    //DEBUG ("interrupt %d", regs->int_no);
 
        if(ActiveInterruptManager == 0)
            {
                DEBUG("not activate InterruptManager vector:%x\t", regs->int_no);
                return;
            }
        ActiveInterruptManager->DoHandleInterrupt(regs);
        return ;
    
    DEBUG ("activate=false, interrupt disable\n");
   // return 0;
}

void InterruptManager::DoHandleInterrupt(isr_ctx_t *regs)
{
    if(regs->int_no != 0x2e && regs->int_no !=0x2c && regs->int_no != 0x22)
        
        DEBUG("interupt:%x,  ", regs->int_no);
  
    if (handlers[regs->int_no])
    { 
        handlers[regs->int_no]->HandleInterrupt(regs);
    }
    else
    {
        isr_handler(regs);      
...
void InterruptHandler::HandleInterrupt(isr_ctx_t *regs)
{
