...
namespace TSS
{
    void InitializeTSS(tss_t* tss, void* gdt){
        ...
        tss->ist1 = (uint64_t)malloc(4096);//Memory::KernelAllocate4KPages(8);
        tss->ist2 = (uint64_t)malloc(4096);//Memory::KernelAllocate4KPages(8);
        tss->ist3 = (uint64_t)malloc(4096);//Memory::KernelAllocate4KPages(8);
